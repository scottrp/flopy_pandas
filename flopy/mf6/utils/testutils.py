...
def read_sfr_rec(sfr_file, cellid_size=3, tuple_cellid=True):
    fd = open(sfr_file, "r")
    sfrrecarray = []
    for line in fd:
        fd_spl = line.strip().split()
        try:
            ...
            if tuple_cellid:
                ...
                if fd_spl[1].lower() != "none":
                    cellid = []
                    for idx in range(1, 1+cellid_size):
                        cellid.append(int(fd_spl[idx]) - 1)
                else:
                    cellid = [fd_spl[1]]
                    temp_size = 1
        except ValueError:
            cellid = fd_spl[1]
            temp_size = 1
        begin = [int(fd_spl[0]) - 1]
        end = [float(fd_spl[temp_size + 1]),
        ...
        full = [*begin, *cellid, *end]
        sfrrecarray.append(full)
    fd.close()
    return sfrrecarray
    ...
    for line in fd:
        ...
        for item in fd_spl[2:]:
            if fd_spl[1].lower() == "diversion" and first:
                per_arr.append(int(item) - 1)
                first = False
            elif fd_spl[1].lower() == "status":
                per_arr.append(item)
            else:
                per_arr.append(float(item))
