...
class BasicListStorage:
    ...
    def internal_size(self):
        if not isinstance(self.internal_data, pandas.DataFrame):
            ...
            return self.internal_data.shape[0]
                ...
                if data_item.type == DatumType.keyword:
                    ...
                    if data_item.name.lower() == "cellid":
                        if isinstance(self._mg, StructuredGrid):
                            ...
                            self._append_type_list("col", i_type)
                        elif isinstance(self._mg, VertexGrid):
                            self._append_type_list("layer", i_type)
                    ...
                    raise FlopyException("ERROR: Data type {data_item.type} "
...
#            data = pandas.DataFrame(data, self._header_names).set_index('f0')
        ...
        if isinstance(data, pandas.DataFrame):
            ...
            for header_column, data_column_type, data_column_name in \
                    zip(self._data_header, data_types.values, data.columns):
                # verify column types
            ...
            if len(col_rename) > 0:
                ...
                data.rename(columns=col_rename)
    ...
    def _get_pandas_dtype(self, str_dtype):
        if str_dtype == "int64":
            return np.int64
    ...
    def _read_data_header(self, data_line):
        ...
        names = []
        dtypes = {}
        num_rows = int(data_line_lst[-2])
        for entry in data_line_lst[1:-2]:
            name, dtype = entry.split(":")
            names.append(name)
            dtypes[name] = self._get_pandas_dtype(dtype)
            # TODO: verify correct names and types

        return names, dtypes, num_rows

    def _read_text_data(self, fd_data_file, first_line, external_file=False):
        # read data header
        if first_line is None:
            first_line = fd_data_file.readline()
        names, dtypes, num_rows = self._read_data_header(first_line)
        ...
        if names is None:
            ...
            data_frame = pandas.read_csv(fd_data_file, sep=" ",
                                         ...
                                         nrows=num_rows, index_col=False,
    ...
    def _write_file_entry(
        ...
        if (data_storage.data_storage_type == DataStorageType.internal_array
            ...
            if data_storage.data_storage_type == \
                ...
                header = f'  # "{column_def_text}" {num_rows} fph\n'
                # write header
