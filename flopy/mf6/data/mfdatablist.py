...
class MFBasicList(mfdata.MFMultiDimVar, DataListInterface):
    ...
    @staticmethod
    def _get_type_pandas(pdata):
        current_type = []
        if len(pdata) == 0:
            return current_type
        for datum in pdata.iloc[0].tolist():
            current_type.append(type(datum))
        return tuple(current_type)
    ...
    def _set_data(self, data, check_data=True):
        ...
        if isinstance(data, pd.DataFrame):
            data = {self._get_type_pandas(data): data}
        elif isinstance(data, np.recarray) and self._header_names is not None:
            # verify data shape
...
#            data = pandas.DataFrame(data).set_index('f0')
            ...
            if isinstance(data[0], pd.DataFrame):
                data_dict = {}
                for item in data:
                    data_dict[self._get_type_pandas(item)] = item
                data = data_dict
            else:
...
#            data = pandas.DataFrame(data, self._header_names).set_index('f0')
    ...
    def _increment_id_fields(self, data_frame):
        for id_field in self._get_id_fields(data_frame):
            if id_field in data_frame:
                data_frame.loc[data_frame[id_field].ge(0), id_field] += 1
                data_frame.loc[data_frame[id_field].lt(0), id_field] -= 1
                #data_frame[id_field] += 1

    def _decrement_id_fields(self, data_frame):
        for id_field in self._get_id_fields(data_frame):
            if id_field in data_frame:
                data_frame.loc[data_frame[id_field].gt(0), id_field] -= 1
                data_frame.loc[data_frame[id_field].le(0), id_field] += 1
                #data_frame[id_field] -= 1
    ...
    def _write_file_entry(
        ...
        for idx, int_data in enumerate(data_storage.internal_data.values()):
            ...
            if data_storage.data_storage_type == \
                    DataStorageType.internal_array or not data_storage.binary:
        ...
        for idx, data in enumerate(data_list):
            ...
            if (data_storage.data_storage_type ==
                ...
                if data_storage.data_storage_type == \
                    ...
                    fd_data_file.write(headers[idx])
                    ...
                    # clean up
                    # TODO: This does not appear to be dropping the columns
        ...
        if data_storage.internal_data is not None:
            # clean up
            for key, int_data in data_storage.internal_data.items():
                if "leading_space" in int_data:
                    int_data = int_data.drop(columns="leading_space")
                if "leading_space_2" in int_data:
                    int_data = int_data.drop(columns="leading_space_2")
                data_storage.internal_data[key] = int_data
        return result
