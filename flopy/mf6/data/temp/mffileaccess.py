...
class MFFileAccessList(MFFileAccess):
    ...
    def read_binary_data_from_file(
        self, read_file, modelgrid, precision="double", inc_index=True,
    ):
        # read from file
    ...
    def write_binary_file(
        ...
        if isinstance(data, pandas.DataFrame):
            ...
            col_dtypes = {}
            for col in header:
                col_dtypes[col[0]] = col[1]
            data_array = data.to_records(index=False, column_dtypes=col_dtypes)
        else:
    ...
    def _get_cell_header(self, modelgrid):
        if modelgrid.grid_type == "structured":
            return [("layer", np.int32), ("row", np.int32), ("column", np.int32)]
