...
class MFBlock:
    ...
    def data_factory(
        ...
        if (
            ...
            # self._container_package.structure.basic_package and \
            if structure.basic_item() and \
                    self._container_package._package_type != "oc" and \
                    self._container_package._package_type != "nam" and \
                    self.structure.name.lower() != "options" and \
                    self._simulation_data.pandas_support:
                ts_names = self._dimensions.get_tsnames()
                tas_names = self._dimensions.get_tasnames()
                if not (len(ts_names) > 0 or len(tas_names) > 0):
                    return mfdatablist.MFBasicList(
                        sim_data,
                        model_or_sim,
                        structure,
                        data,
            ...
            if structure.basic_item() and \
                ...
                tas_names = self._dimensions.get_tasnames()
                if not (len(ts_names) > 0 or len(tas_names) > 0):
                    trans_list = mfdatablist.MFBasicTransientList(
                        sim_data,
                        model_or_sim,
                        structure,
                        enable,
            ...
            if (
                ...
                or (
                    (isinstance(dataset, mfdatalist.MFList) or
                     isinstance(dataset, mfdatablist.MFBasicList))
    ...
    def set_all_data_internal(self, check_data=True):
        ...
        for key, dataset in self.datasets.items():
            if (
                ...
                or (
                    (isinstance(dataset, mfdatalist.MFList) or
                     isinstance(mfdatablist.MFBasicList))
                    and dataset.structure.type == DatumType.recarray
                )
                and dataset.enabled
            ):
    ...
    def _prepare_external(self, fd, file_name, binary=False):
        ...
        try:
            fd_ext = open(fd_file_path, file_opts)
        except:
            type_, value_, traceback_ = sys.exc_info()
            message = (
                f'Error writing external file '
                f'"{file_name}"'
            )
            raise MFDataException(
                self._container_package.model_name,
        ...
        return fd_main, fd_ext

    def _write_block(self, fd, block_header, ext_file_action):
        transient_key = None
        basic_list = False
        dataset_one = list(self.datasets.values())[0]
        if isinstance(dataset_one, mfdatablist.MFBasicList) or \
                isinstance(dataset_one, mfdatablist.MFBasicTransientList):
            basic_list = True
            for dataset in self.datasets.values():
                assert isinstance(dataset, mfdatablist.MFBasicList) or \
                    isinstance(dataset, mfdatablist.MFBasicTransientList)
            # write block header
            block_header.write_header(fd)
        # elif len(block_header.data_items) > 0:
            # write block header
        #    block_header.write_header(fd)
                        ...
                        if ext_fname is not None:
                            if dataset.has_modified_ext_data():
                                ...
                                if not binary:
                                    ...
                                    header = (
                                        "# File generated by Flopy version {} "
                                        "on {} at {}."
                                        "\n".format(
                                            __version__,
                                            dt.strftime("%m/%d/%Y"),
...
class MFPackage(PackageContainer, PackageInterface):
    ...
    def __setattr__(self, name, value):
        if hasattr(self, name) and getattr(self, name) is not None:
            ...
            if attribute is not None and isinstance(attribute, mfdata.MFData):
                try:
                    if isinstance(attribute, mfdatalist.MFList) or \
                            isinstance(attribute,
                                       mfdatablist.MFBasicList):
    ...
    def inspect_cells(self, cell_list, stress_period=None):
        ...
        for block in self.blocks.values():
            ...
            for dataset in block.datasets.values():
                if isinstance(dataset, mfdatalist.MFList) or \
                        isinstance(dataset, mfdatablist.MFBasicList):
                    # handle list data
                    cellid_column = None
                    ...
                    if cellid_column is not None:
                        ...
                        local_index_cells = []
                        # get data
                        if isinstance(dataset, mfdatalist.MFTransientList) or \
                                isinstance(dataset,
                                           mfdatablist.MFBasicTransientList):
            ...
            if isinstance(data, dict):
                ...
                for order, key, value in ordered_data_items:
                    ...
                    if isinstance(key, str) and hasattr(package, key):
                        ...
                        if isinstance(child_data_attr, mfdatalist.MFList) or \
                                isinstance(child_data_attr,
                                           mfdatablist.MFBasicList):
